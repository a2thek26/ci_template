...
class Carabiner {
  ...
  public function has_files($type, $group = 'main')
  {
    switch ($type)
    {
      case 'css':
      case 'CSS':
        if (isset($this->css[$group]) && count($this->css[$group]) > 0) {
          return true;
        } else {
          return false;
        } // end if 
      ...
      case 'js':
      case 'JS':
        if (isset($this->js[$group]) && count($this->js[$group]) > 0) {
          return true;
        } else {
          return false;
        } // end if 
	...
	*/	
	...
	{	
		...
		$this->script_path = APPPATH . '../' . $this->script_dir;
		$this->script_uri = $this->base_uri.$this->script_dir;
		
		$this->style_path = APPPATH . '../' . $this->style_dir;
		$this->style_uri = $this->base_uri.$this->style_dir;

		$this->cache_path = APPPATH . '../' . $this->cache_dir;
	...
	*/
	private function _get_contents($ref)
	{
		...
		return $this->_replaceContent($contents);
		
	}
        
        private function _replaceContent($contents)
        {
          foreach ($this->replace_values as $string => $value) {
            $contents = str_replace($string, $value, $contents);
          } // end foreach replace string
